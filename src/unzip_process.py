...
from src.clean_gha import get_ck_client, get_ck_conn_info, insert_into_ck
...
for result in results:
    ...
    if state == 'unzip' and is_success ==0:
        not_unzip_file_list.append((year, month, day, hour))
print(file_state_map)
...
for file in not_unzip_file_list:
    ...
    if file_state_map.get(year + '_' + month + '_' + day + '_' + hour + '_' + 'download') == 1 and file_state_map.get(
            year + '_' + month + '_' + day + '_' + hour + '_' + 'insert') == 0:
        if int(month) < 10:
            month = '0' + month
        if int(day) < 10:
            day = '0' + day
        file_path = root_path + '/' + year + '/' + year + '-' + month + '-' + day + '-'+hour+'.json.gz'
